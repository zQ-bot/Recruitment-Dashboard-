    ...
    <div className="grid min-h-screen grid-cols-1 lg:grid-cols-[236px_1fr]">
      ...
      <main className={`min-w-0 ${blindMode ? "blind" : ""}`}>
        ...
        <div className="px-5 pb-14 pt-6 md:px-7">
          {view === "overview" && <OverviewView onOpenCandidate={openCandidate} searchQuery={searchQuery} statusFilter={statusFilter} />}
          {view === "pipeline" && <PipelineView onOpenCandidate={openCandidate} searchQuery={searchQuery} statusFilter={statusFilter} />}
          {view === "viewer" && (
            <ResumeViewerView activeId={activeCandidate} onSelect={setActiveCandidate} onExport={setExportId} searchQuery={searchQuery} statusFilter={statusFilter} />
          )}
          {view === "interviews" && <InterviewPlanningView searchQuery={searchQuery} statusFilter={statusFilter} />}
